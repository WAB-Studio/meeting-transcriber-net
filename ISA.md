...
- [x] ISC-162: Anti: a machine that has not come back from one of those questions is asked no more of them until it does, so that wait is paid once rather than every time something looks.
...
## Verification
- ISC-161 — `DeviceEnquiryTests` (`tests/MeetingTranscriber.Audio.Tests`) green 2026-08-20, over bodies doing what no build agent's audio service can be asked to do: stay inside the call. Asking what this machine can record from comes back at the deadline rather than never, says what it was asked about, and the body is still in there when it says so. Red 2026-08-20 with the question run on the caller's thread instead, which is what listing did until now: the six-test run was killed at 120 s with `.A_machine_that_has_not_come_back_is_asked_nothing_else` still going at 1 m 57 s, where bounded they take 20 s — the freeze while somebody chooses a microphone, as the card describes it. Two more hold what the shape must not cost: a machine that answers pays no deadline and hands back what it said, and one that refuses throws Windows' own exception unwrapped, so a machine that said no is never reported as one that said nothing and is never remembered as a question still out there. That both questions this application asks about devices are behind the ask rather than beside it is `.Both_questions_this_application_asks_about_devices_go_through_the_deadline`, which reaches them with no device on the agent: with a question still out there each comes back at once, which only going through the ask can do. Red 2026-08-20 with the remembering taken out, where `AudioDevices.Microphones()` enumerated this machine's real endpoints and answered. What no probe reaches is that every call inside those two bodies is inside the ask — the enumerator, the default endpoint and each driver's property store — read off `AudioDevices`, since forcing it needs the wedged audio service ISC-129 to ISC-131 wait on for a device. The deadline is kept out of that file's one funnel deliberately: `AudioDevices.Open` and `AudioDevices.EngineFormat` run inside a `DeviceOpen` ask that is already counting, and one there would be two deadlines over a single device
- ISC-162 — `DeviceEnquiryTests.A_machine_that_has_not_come_back_is_asked_nothing_else` and `.A_machine_that_comes_back_is_asked_again` (`tests/MeetingTranscriber.Audio.Tests`) green 2026-08-20: with a question still out there the next one is refused at once, is never put to the machine at all, and names the question still out there rather than the one just asked — and once the abandoned body comes back, asking resumes, so this is a machine given up on and never Windows given up on. Red 2026-08-20 with the remembering taken out and the deadline left in: 2 of the 6 failed, the second question waiting five seconds of its own and reaching the machine. That is what the deadline alone does not buy, and why this is a claim rather than a detail — a screen asks once a second and the watcher every two, so what a person would sit through is the same freeze with pauses in it, one abandoned thread per go
