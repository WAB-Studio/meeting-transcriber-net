...
- ISC-152 — `UiTextsTests`, `UiTextTests`, `TextLineTests` and `DeviceLinesTests` (`tests/MeetingTranscriber.Presentation.Tests`), `ScreenTextsTests` and `MeetingCardTextTests` (`tests/MeetingTranscriber.App.Tests`) and `WaitingRecordingsTests` (`tests/MeetingTranscriber.Recording.Tests`) green 2026-09-02, re-run because the claim was false twice: `WaitingRecording.Unrecoverable` handed the meetings list English prose, closed on `MeetingCardTextTests`; the microphone picker read `(default)` in Spanish off `AudioDevice.ToString`, closed on `DeviceLinesTests` and `ScreenTextsTests.No_screen_shows_a_person_what_a_type_says_about_itself`. Then the UI probe on the packaged build 2026-09-02, the application in Spanish: `choose MicrophonePicker "(default)"` failed listing `Micrófono (fifine Microphone) (predeterminado)`. `ScreenTextsTests.No_screen_names_anything_but_an_entry_in_the_catalogue` re-run 2026-09-02 holding a binding to the whole `{x:Bind In(loc:UiTexts.…)}` form, red on a planted `{x:Bind TheOtherSide}` and on a planted `<Setter Property="Text" Value="Sin nombre" />`, green before both. Not reached: a `ToString` behind a method, which is `SourcePicker` today, and a `Setter` in `Olivo.xaml` over a property a person reads.
...
- ISC-170.1 — `MeetingsWatchTests` (`tests/MeetingTranscriber.Recording.Tests`) green 2026-09-02, and the UI probe on the packaged build that day: `see a1-before sleep 20 see a2-during sleep 48 see a3-after`, with `record --corpus <the corpus> --language es --seconds 35` at a prompt after the first tree. Meeting `2e581a94` is absent from that tree, reads `It is being recorded right now.` in the second, and in the third reads `2026-09-02 18:02 · 0:00:35`, `Recorded.` with Transcribe — nothing pressed, nothing started again, the count 7 to 8. Red that day three times: the stage and standing out of the comparison, the recordings out of it, the list's own read not fed back. No probe reaches a job finishing on its own because nothing runs one; the stage moved here is a recording filed, which moves the same two terms
