...
- ISC-93 — `CorpusSearchTests` (`tests/MeetingTranscriber.Infrastructure.Tests`) green 2026-09-10 over each of the five things `arquitectura.md` §8.1 promises: `.A_meetings_own_words_are_something_search_finds`, `.A_meeting_is_found_by_the_thing_it_is_filed_under`, `.A_meeting_is_found_by_the_organization_above_what_it_is_filed_under`, `.A_meeting_is_found_by_somebody_it_names`, `.A_decision_an_action_and_an_open_question_are_each_found_where_they_were_said`, `.Only_the_extraction_a_person_accepted_last_answers` and `.No_source_takes_the_whole_page_from_the_others`, with `CorpusMigrationTests.An_index_that_arrives_after_the_rows_holds_them` for a corpus that had rows before the indexes. Red 2026-09-10 one mutation at a time: five of the six seeds deleted reddened the migration test; the node branch ordered flat answered an initiative's name with twenty node rows and no turns. Found and not merely indexed is the ordering: `bm25` ranks inside an index and not across, so the page takes the best of each source in turn. No probe reaches the MCP server or the CLI's own labelling
