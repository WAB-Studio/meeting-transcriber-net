...
- [x] ISC-177: Anti: a corpus removes without being asked only an abandoned write and a copy whose replacement is back in place — never the only copy left of a file that is missing, and never a write in progress.
- [x] ISC-181: A copy a finished replace left behind is cleared by the corpus's own tidy-up, so a render the machine died inside does not leave it reading unsound until somebody deletes a file by hand.
...
- [x] ISC-184: What a meeting's channel stopped following mid-recording, and from when, is in the corpus once the recording's own folder is gone.
...
- [x] ISC-156.1: Anti: a meeting no sample was ever captured for does not survive as one — its row and its folder are gone the next time the application opens.
...
- ISC-8 — `JobStateTests.Only_the_states_the_runner_owns_are_picked_up_by_itself` (`tests/MeetingTranscriber.Domain.Tests`) green 2026-08-07, and `JobRunnerTests.Nothing_waiting_on_a_person_is_ever_started_by_itself` (`tests/MeetingTranscriber.Processing.Tests`) green 2026-09-25
...
- ISC-22 — `CorpusIsOneThingTests.A_corpus_says_which_folder_it_is` green 2026-08-13, and `CorpusIsOneThingTests` in `Cli.Tests` and `CorpusImport.Tests` (that suite deleted with the importer) green 2026-08-13, each red against a signature taking both in the assemblies it covers
...
- ISC-34 — `CliWalkthroughTests.The_same_response_imported_twice_is_one_meeting`, `AudioIntakeTests.The_same_audio_brought_in_twice_is_one_meeting` (both a single track and a pair, compared after the mix down because that is what would land), `.The_same_audio_under_another_folder_name_is_still_one_meeting`, `ImportAudioCommandTests.Bringing_the_same_audio_in_twice_is_one_meeting`, `MeetingIntakeTests.A_response_for_a_meeting_this_corpus_recorded_leaves_one_meeting` and `.The_same_response_filed_onto_a_meeting_twice_is_one_filing` (`tests/MeetingTranscriber.Cli.Tests`, `tests/MeetingTranscriber.Recording.Tests`, `tests/MeetingTranscriber.Processing.Tests`) green 2026-09-16. The last two are the third door: a response filed onto a meeting this corpus recorded. The folder-name clause has a probe again — it moves the folder and the file name together — the one it replaced having gone with the Python importer on 2026-08-26. What none of them reaches is a recorded meeting's own folder, card and all, arriving under a new name — identity is the hash of what would land there too, and nothing measures it
- ISC-35 — `CorpusRebuildTests.Deleting_every_derived_row_and_projecting_again_leaves_every_other_table_as_it_was` (`tests/MeetingTranscriber.Infrastructure.Tests`) green 2026-08-07, which holds the classifications and the speaker assignments a person edited as well as the rows nothing touched
- ISC-36 — `CorpusRebuildTests.Rebuilding_produces_the_same_projections_and_the_same_files` (`tests/MeetingTranscriber.Processing.Tests`) and `MeetingRendererTests.Rendering_again_leaves_the_sources_alone_and_produces_the_same_files` green 2026-08-07
- ISC-37 — `CorpusRebuildTests.A_claim_cannot_cite_a_turn_the_meeting_never_had` (`tests/MeetingTranscriber.Infrastructure.Tests`) green 2026-08-07. That half only: the deleted-out-from-under-it half held for a rebuild that finishes and not for one refused partway, where the turns went and the claims stayed. `CorpusRebuildTests.A_meeting_refused_with_cited_turns_costs_that_meeting_and_not_the_run` (`tests/MeetingTranscriber.Processing.Tests`) green 2026-09-02 is what reaches it, red that day against the projection deleting before it knew the new turns would save
...
- ISC-91 — `CorpusRebuildTests.A_second_extraction_leaves_the_first_ones_state_alone_and_starts_its_own_blank` (`tests/MeetingTranscriber.Infrastructure.Tests`) green 2026-08-07
...
- ISC-96 — `CorpusSearchTests.Throwing_every_index_away_and_rebuilding_them_answers_exactly_the_same` and `CorpusIntegrityTests.Compacting_leaves_search_answering_exactly_what_it_answered_before` green 2026-08-07
...
- ISC-123 — `UnfinishedRecordingsTests.Every_recording_nobody_stopped_is_found_again_with_what_it_is_and_what_is_in_it` and `.A_recording_with_no_card_is_offered_all_the_same` (`tests/MeetingTranscriber.Audio.Tests`) green 2026-08-15, over a root holding one recording with both sources, one with a single source and a folder that is not a recording at all. `RecoveryCommandTests.What_is_waiting_says_which_meeting_it_is_and_what_each_source_holds` (`tests/MeetingTranscriber.Cli.Tests`) holds it at the surface a person meets it at
- ISC-124 — one fact per outcome (`tests/MeetingTranscriber.Audio.Tests`, `tests/MeetingTranscriber.Cli.Tests`) green 2026-09-16: kept, `UnfinishedRecordingsTests.A_recording_that_is_kept_says_what_survived_and_is_still_there_afterwards`; taken out, `.A_recording_whose_audio_is_taken_out_lands_where_it_was_asked_for_and_stays_put`; thrown away, `.A_recording_that_is_thrown_away_is_gone_with_everything_in_it`. Taking audio out of a recording the disk damaged is its own answer: every source that poured lands where it was asked for and its blocks stay, while the one whose blocks will not read through costs its own file, is named on a `not read:` line of its own and brings the command back Refused — `.Audio_taken_out_leaves_what_poured_when_a_later_source_cannot_be_read`, `RecoveryCommandTests.A_source_that_will_not_read_says_so_and_what_poured_stays_where_it_poured` and `CorpusRecoveryCommandTests.A_source_that_will_not_read_costs_its_own_file_and_says_so_at_the_prompt`. `RecoveryCommandTests.Deciding_nothing_about_a_recording_is_a_misuse` and `.Deciding_two_things_about_a_recording_is_a_misuse` hold that there is no default
- ISC-125 — `UnfinishedRecordingsTests.Nothing_but_a_decision_about_one_recording_removes_a_folder` and `.Nothing_in_the_audio_engine_removes_a_file_it_did_not_just_create` green 2026-08-15, each red 2026-08-15 against a `Directory.Delete` and a `Delete(` planted in a source file that has no business holding one. Widened 2026-09-07 to `.Nothing_removes_or_renames_through_a_handle` and `.Every_way_a_folder_goes_is_read_as_the_way_it_is`: red that day against a handle delete and a handle rename planted in `src/MeetingTranscriber.Recording/MeetingsNobodyRecorded.cs`, against a `Directory.Delete` planted in `src/MeetingTranscriber.Audio/CaptureSession.cs`, against each way and the lookbehind dropped from the pattern, against an allow entry holding nothing, and against a sweep reading no file. Both arms need a parenthesis, so a method group, a `using static` and a P/Invoke are reached by no guard — narrower there than the sweep this replaced
...
- ISC-126.1 — `SavingMarkTests.A_save_that_is_running_holds_the_folder_and_refuses_a_second_one`, `UnfinishedRecordingsTests.None_of_the_three_outcomes_lands_on_a_recording_whose_save_is_running` (`tests/MeetingTranscriber.Audio.Tests`), `WaitingRowsTests.A_meeting_whose_save_is_running_is_never_offered_as_one_to_decide_about`, `.A_meeting_whose_devices_are_still_closing_on_a_save_is_a_meeting_being_saved`, `WaitingRecordingsTests.Nothing_else_is_offered_the_meeting_a_save_is_making_while_it_is_making_it` and `.A_second_save_of_the_same_recording_is_refused_rather_than_run_beside_the_first` (`tests/MeetingTranscriber.Recording.Tests`), and at the prompt `CorpusRecoveryCommandTests.A_meeting_whose_save_is_running_is_listed_with_none_of_the_three_offered`, `.None_of_the_three_lands_on_a_meeting_whose_save_is_running` and `RecoveryCommandTests.None_of_the_three_lands_on_a_folder_whose_save_is_running` (`tests/MeetingTranscriber.Cli.Tests`) green 2026-09-02, the fourth reading the list from a second context inside a finish that is really running. Red 2026-09-02 with the standing asked capture-first, 1 failing, and with the finish claiming nothing, 2 failing. Unreached: a window, which needs a UI thread and a packaged host, and whose list is built on a press — so a save another process starts leaves the answers drawn, refused when pressed, until the next one; a second process; and the minutes a long save runs, sampled here at one instant
- ISC-126.2 — `SavingMarkTests.A_save_the_process_died_in_the_middle_of_leaves_the_recording_decidable_again`, `.A_mark_a_save_left_behind_holds_the_recording_for_nobody`, `UnfinishedRecordingsTests.A_mark_left_by_a_save_that_died_leaves_the_recording_decidable` (`tests/MeetingTranscriber.Audio.Tests`), `WaitingRowsTests.A_meeting_whose_save_is_running_is_never_offered_as_one_to_decide_about` (`tests/MeetingTranscriber.Recording.Tests`), `CorpusRecoveryCommandTests.A_meeting_whose_save_died_is_offered_again_rather_than_held_by_what_it_left` and `RecoveryCommandTests.A_mark_a_save_left_behind_keeps_none_of_the_three_from_a_folder` (`tests/MeetingTranscriber.Cli.Tests`) green 2026-09-02: the first kills a second process holding the mark, the rest run over the file that stranding leaves. Red 2026-09-02 with the mark read as a file that is there rather than one something holds — 9 failed across those three projects, `WaitingRecordingsTests.A_finish_that_was_cut_off_after_filing_the_audio_is_still_waiting_and_completes` among them, which is this claim's own shape. Two things unreached: no probe cuts a machine's power, which strands the same file the same way, and none runs on a file system that does not enforce share modes, where every reader would answer that no save is running
...
- ISC-152 — `UiTextsTests`, `UiTextTests`, `TextLineTests`, `DeviceLinesTests` (`tests/MeetingTranscriber.Presentation.Tests`), `ScreenTextsTests`, `MeetingCardTextTests` (`tests/MeetingTranscriber.App.Tests`), `WaitingRecordingsTests` (`tests/MeetingTranscriber.Recording.Tests`) green 2026-09-02, after two falsifications: `WaitingRecording.Unrecoverable`, closed on `MeetingCardTextTests`; `AudioDevice.ToString`, closed on `DeviceLinesTests` and `ScreenTextsTests.No_screen_shows_a_person_what_a_type_says_about_itself`. UI probe, packaged build 2026-09-02: `choose MicrophonePicker "(default)"` failed listing `Micrófono (fifine Microphone) (predeterminado)`. `ScreenTextsTests.No_screen_names_anything_but_an_entry_in_the_catalogue` red 2026-09-02 on a planted `{x:Bind TheOtherSide}` and on `<Setter Property="Text" Value="Sin nombre" />`. `OlivoTests.The_dictionary_settles_no_word_a_person_reads_through_a_style` and `.Every_property_a_person_reads_is_caught_by_a_setter_row` green 2026-09-04 (`dotnet test tests/MeetingTranscriber.App.Tests`), red on that `Setter` spliced into `Olivo.xaml` and re-parsed, on a row dropped, on `Watermark` added to `ScreenTextsTests.Reads`, and on either arm of `OlivoTests.IsOneOf`; `ScreenTextsTests.A_screen_naming_anything_else_is_found` red on `Property="TextBlock.Text"`. Not reached: `SourcePicker.ToString`; `<Setter Target="…">` in `VisualState.Setters`, ten in `Olivo.xaml`; `<ContentPresenter Content="Guardar" />`.
...
- ISC-156.1 — `MeetingsNobodyRecordedTests` (`tests/MeetingTranscriber.Recording.Tests`) and `CaptureMarkTests` (`tests/MeetingTranscriber.Audio.Tests`) green 2026-09-02: a press whose devices never recorded loses its folder and its row, and a recording holding one block keeps every byte, hashed before and after. Red 2026-09-02 four ways — a spool counted by existing rather than by holding a block, which is what a kill during device-open leaves; the sweep taking every folder rather than what recovery left over; the row removed before the folder; and the mark read as being there rather than held. No probe reaches the press itself, which needs two devices and a window
...
- ISC-157 — `MeetingRecordingsTests.Stopping_a_recording_queues_no_work_on_the_meeting`, `.Stopping_a_recording_somebody_asked_to_transcribe_queues_exactly_that` and `.Stopping_a_recording_somebody_asked_to_summarise_queues_only_the_transcription`, with `WhatStoppingStartsTests` over the rule that answers and `CorpusSettingsTests` over what an unanswered corpus reads as (`tests/MeetingTranscriber.Recording.Tests`, `tests/MeetingTranscriber.Infrastructure.Tests`) green 2026-09-16. It takes the pair to hold the sentence now that somebody can ask beforehand: with nothing asked for, `processing_jobs` is empty read back through a second connection; with transcribing asked for it holds that and nothing else, so what the first fact measures is a choice and no longer an absence. Not reached: the settling done on the screen rather than in the corpus, and that what was asked for then happens — ISC-157.1, open: transcribing does, by `AfterAStopTests` (`tests/MeetingTranscriber.Recording.Tests`); summarising waits on F6
...
- ISC-177 — `ArtifactReconcilerTests.A_copy_is_taken_only_where_the_file_that_replaced_it_is_back`, `.A_file_this_corpus_did_not_set_aside_is_not_a_copy_of_anything`, `.A_copy_the_disk_marked_read_only_is_still_taken` and `DurableWriteTests.A_replace_that_stopped_partway_leaves_a_copy_the_sweep_finishes_with` (`tests/MeetingTranscriber.Infrastructure.Tests`) green 2026-09-02, each red that day on one reverted line: the sweep reading the suffix alone, the destination read off the last full stop, the read-only bit left standing. The write half of the claim rests on `DurableWriteTests.A_sweep_running_beside_a_write_leaves_the_write_alone`, green and unchanged since #241 rather than re-reddened here. Outside all of it: the audio engine writes `.partial` files nothing holds, and no probe schedules a sweep and a replace at once, so those windows are read off the code.
...
- ISC-173.1 — `OlivoTests.No_screen_chooses_a_colour_a_size_or_a_corner_of_its_own`, `.No_code_behind_chooses_a_colour_or_a_size_of_its_own`, `.No_screen_quietens_a_line_of_text_with_opacity`, `.Every_resource_a_screen_names_is_one_that_exists`, `.Every_resource_the_code_behind_asks_for_by_name_exists` and `.The_dictionary_carries_every_colour_the_design_names_at_the_value_it_names` (`tests/MeetingTranscriber.App.Tests`) green 2026-09-02, the last reading the colour table out of `docs/design.md` and not out of the dictionary it checks. Red that day over a `FontSize`, a `Background` and a glyph size in the saving card. All six re-run green 2026-09-04 (`dotnet test tests/MeetingTranscriber.App.Tests`), the first now reading a `<Setter.Value>` and a qualified `Property`, red on `<Setter Property="Background"><Setter.Value>#FF112233</Setter.Value></Setter>` spliced into `ClassifyingAMeeting.xaml` and re-parsed. Not reached: whether a named key resolves at run time, which no build agent can open a window to see — the probe is what showed it does; a property element other than `<Setter.Value>`; `<Setter Target="…">`; and whether `<StaticResource ResourceKey="…" />` under a `<Setter.Value>` names a key that exists, which `Named()` does not match
- ISC-170.1 — `MeetingsWatchTests` (`tests/MeetingTranscriber.Recording.Tests`) green 2026-09-03, and the UI probe on the packaged build 2026-09-02: `see a1-before sleep 20 see a2-during sleep 48 see a3-after`, with `record --corpus <the corpus> --language es --seconds 35` at a prompt after the first. Meeting `2e581a94` is absent from that tree, reads `It is being recorded right now.` in the second, and `2026-09-02 18:02 · 0:00:35`, `Recorded.` with Transcribe in the third — nothing pressed, nothing relaunched, 7 rows to 8. Red four times: 2026-09-02 the stage and standing out of it, the recordings out of it, the list's own read not fed back; 2026-09-03 `A_change_that_landed_while_the_list_was_drawing_is_still_told_about`, over a watch answering that read with one of its own. Not reached: a job the runner finished while a list was open — the stage moved here is a recording filed, moving the same two terms
- ISC-150.1 — `RecordingMetersTests.Nothing_is_said_about_the_speakers_before_the_machine_has_answered`, `.What_the_machine_last_said_about_the_speakers_stands_when_it_stops_answering`, `.A_machine_that_answers_again_replaces_what_was_standing` and `.What_the_last_meeting_was_told_does_not_stand_for_the_next_one` (`tests/MeetingTranscriber.Recording.Tests`) green 2026-09-09. Red that day one mutation at a time: a refused ask inventing a default reddened the first, second and fourth; clearing the standing answer, the second; catching only `AudioDeviceWedgedException`, the second; writing only while it is nothing, the third; forgetting nothing between meetings, the fourth. What no probe reaches is the window itself, which needs a UI thread and a packaged host, so that it hands the rule what it reads and asks the machine nowhere else is held by the compiler and by a person running the packaged build
- ISC-84 — `DeepgramKeyTests` (`tests/MeetingTranscriber.Infrastructure.Tests`) and `KeyCommandTests` (`tests/MeetingTranscriber.Cli.Tests`) green 2026-09-09 against the real Windows credential store, on a target of each test's own. Red that day, seventeen mutations one at a time, each at the test named for it: `.Nothing_but_the_key_itself_reaches_the_credential_store` sweeps `src/` and finds one file reaching the store, red with a second; `.Nothing_but_the_key_itself_reads_a_Deepgram_key` finds the type in two, red with a third; `.A_key_typed_after_an_equals_sign_is_refused_without_being_repeated` red with the command table not telling `Arguments.Parse` the line can carry a key. No probe reaches a Deepgram call — `JobRunner` spends through `TranscribingOnThisMachinesKey`, and no suite may call the provider — so this is a sweep over source, not the walk of a transcription the card asks for
- ISC-93 — `CorpusSearchTests` (`tests/MeetingTranscriber.Infrastructure.Tests`) green 2026-09-16 over each of the six things `arquitectura.md` §8.1 promises: `.A_meetings_own_words_are_something_search_finds`, `.A_meeting_is_found_by_the_thing_it_is_filed_under`, `.A_meeting_is_found_by_the_organization_above_what_it_is_filed_under`, `.A_meeting_is_found_by_somebody_it_names`, `.A_meeting_is_found_by_the_name_somebody_gave_a_voice`, `.A_decision_an_action_and_an_open_question_are_each_found_where_they_were_said`, `.Only_the_extraction_a_person_accepted_last_answers` and `.No_source_takes_the_whole_page_from_the_others`, with `CorpusMigrationTests.An_index_that_arrives_after_the_rows_holds_them` for a corpus that had rows before the indexes. Red 2026-09-10 one mutation at a time: five of the six seeds deleted reddened the migration test; the node branch ordered flat answered an initiative's name with twenty node rows and no turns. Found and not merely indexed is the ordering: `bm25` ranks inside an index and not across, so the page takes the best of each source in turn. Neither the MCP server nor the command line is held to the six, and both read this same query
- ISC-184 — `MeetingRecordingsTests.What_channel_0_stopped_following_and_when_is_in_the_corpus_with_the_folder_gone` (`tests/MeetingTranscriber.Recording.Tests`) green 2026-09-10, the spool folder deleted before the corpus is read again, and red that day with the write taken out of the finish. Beside it `.A_recording_that_changed_nothing_leaves_nothing_saying_it_did`, `.Finishing_the_same_recording_twice_leaves_one_row_per_move`, `.Both_channels_moving_are_two_facts_with_their_own_instants`, `.A_changes_file_that_cannot_be_read_stops_the_finish_before_anything_is_written` and `CorpusSchemaTests.A_change_on_channel_0_cannot_name_a_device` (`tests/MeetingTranscriber.Infrastructure.Tests`). Not reached: every one fabricates its spools, so a channel 0 moved to the whole machine by somebody mid-meeting is still a run on a machine with two devices
- ISC-144 — `CorpusStatementsTests.A_nodes_own_meetings_come_back_oldest_first`, `.The_three_sections_come_back_interleaved_in_the_order_they_were_said`, `.Only_the_extraction_a_person_accepted_last_answers_about_a_node`, `.A_meeting_on_its_way_out_says_nothing_about_its_node` and `.A_node_this_corpus_does_not_hold_is_refused_by_name` (`tests/MeetingTranscriber.Infrastructure.Tests`), `CorpusNodesTests.The_whole_tree_comes_back_root_first` (`tests/MeetingTranscriber.Infrastructure.Tests`), and `CorpusServerTests.An_agent_finds_a_node_and_reads_the_history_of_everything_under_it`, `.A_node_id_that_is_not_one_is_answered_in_words` and `.A_node_this_corpus_does_not_hold_is_answered_in_words_and_not_with_an_empty_list` (`tests/MeetingTranscriber.Mcp.Tests`) green 2026-09-18. Red one mutation at a time: `kind_rank` or `at_ms` dropped from the ordering moves a row; the newest-first order reddens the oldest-first fact; the existence check dropped answers an unknown node with an empty list; `ClassificationException` off `Answer`'s catch list takes that refusal out of the session as a defect. Not reached: a screen reading this — `leer_nodo`'s answer is read on `nodescreen`'s branch of this same batch, and this claim is about the read and the tool, never the screen
...
- ISC-143 — `MeetingReadingTests.A_meeting_left_without_a_summary_says_which_condition_failed_and_on_which_statement` (`tests/MeetingTranscriber.Infrastructure.Tests`), green 2026-09-26. `MeetingRows.RefusedExtraction` with a `NoEvidence` refusal at `decisions[0]` carrying its statement; `MeetingReading.Of`'s `screen.WhyTheSummaryWasRefused` equals it, and `.A_meeting_that_has_a_summary_says_nothing_about_an_attempt_that_was_refused` (same suite) shows an accepted run afterwards clears it. Red with `MeetingReading.Of` no longer reading the refusal off the newest Extract job's own run. Not reached: the line drawn on a running window, which no build agent can open.
