...
- ISC-152 — `UiTextsTests`, `UiTextTests` and `TextLineTests` (`tests/MeetingTranscriber.Presentation.Tests`), `ScreenTextsTests` and `MeetingCardTextTests` (`tests/MeetingTranscriber.App.Tests`) and `WaitingRecordingsTests` (`tests/MeetingTranscriber.Recording.Tests`) green 2026-09-02, re-run because the claim was false: `WaitingRecording.Unrecoverable` handed the meetings list English prose it printed inside a catalogued frame. What each of them holds is stated in the tests themselves; what closed that hole is `MeetingCardTextTests` reading the reason table, red that day with one arm dropped, one of five declared and unanswered. What no test reaches is the WinUI tree, so what is held instead is that no screen has words of its own to leave behind.
