...
- ISC-166 — `HumanLayerTests` and `MeetingIntakeTests.The_microphones_own_voice_reads_as_whoever_said_they_are_using_this`, `.A_meeting_filed_before_anybody_said_who_is_using_this_names_nobody` and `.Filing_a_response_again_after_answering_settles_what_it_could_not_before` green 2026-09-02 over `tests/fixtures/deepgram/two-channel-one-voice-me.json`, one voice on the microphone against three on the loopback: the transcript reads `## Ada` and leaves `ch0:speaker_0` alone. Asked and kept is the UI probe on the packaged build 2026-09-02, on a corpus with no `is_me` row: the field opens empty, `Save` dead, the question under it; typed and pressed, the name is in `people`; the next start reads it back with the question gone; typed again, that row is renamed and no second person appears. Unreached: a meeting recorded here and transcribed, since nothing in `src/` calls Deepgram — card #111
