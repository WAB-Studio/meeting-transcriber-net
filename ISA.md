...
- [ ] ISC-177: Anti: a corpus removes without being asked only an abandoned write and a copy whose replacement is back in place — never the only copy left of a file that is missing, and never a write in progress.
- [ ] ISC-181: A copy a finished replace left behind is cleared by the corpus's own tidy-up, so a render the machine died inside does not leave it reading unsound until somebody deletes a file by hand.
