...
- **criterion-now** — The corpus is in the user's own profile and never under their application
  data. The whole of that tree is refused, the package container being the case of it an uninstall
  deletes outright rather than redirects into, and the folder the application falls back to is held
  against the folders Windows itself names — so an application data folder redirected somewhere the
  rule's own anchor would not reach fails the assertion too.
