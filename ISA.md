...
## Verification
- ISC-163 — `DeviceEnquiryTests` (`tests/MeetingTranscriber.Audio.Tests`) green 2026-08-20, over bodies doing what no build agent's audio service can be asked to do: stay inside the call. Asking what this machine can record from comes back at the deadline rather than never, says what it was asked about, and the body is still in there when it says so. Red 2026-08-20 with the question run on the caller's thread instead, which is what listing did until now: the six-test run was killed at 120 s with one of them still going at 1 m 57 s, where bounded they take 21 s — the freeze while somebody chooses a microphone, as the card describes it. Two more hold what the shape must not cost: a machine that answers pays no deadline and hands back what it said, and one that refuses throws Windows' own exception unwrapped, so a machine that said no is never reported as one that said nothing and is never remembered as a question still out there. That the two questions asked here on nobody else's behalf are behind the ask rather than beside it is `.Both_questions_this_application_asks_about_devices_go_through_the_deadline`, which reaches them with no device on the agent: one question is wedged at a time and the method that asks that one comes back at once refusing, which only going through the bounded ask, asking that question, can do — so it notices either of the two being taken out from behind the ask, and either being pointed at the other's question. One wedge for both is what stopped working when the memory was scoped, not two wedges at once: a single wedge no longer refuses the other call, and that call would reach the machine, whose answer on a build agent is nobody's evidence. Two wedges overlapping would hold the same assertions for one deadline instead of two, and the five seconds are not worth the thread. Red 2026-08-20 with the memory ignored, where `AudioDevices.Microphones()` enumerated this machine's real endpoints and answered. The thread the questions now run on is in the multi-threaded apartment where a window's were in its own, and the `devices` command run on this machine that day is that path end to end: both endpoints named, the default flagged, and the form factor out of each driver's property store. What no probe reaches is that every call inside those two bodies is inside the ask, or that a third question added beside them would be — read off `AudioDevices`, since forcing it needs the wedged audio service ISC-129 to ISC-131 wait on for a device. The deadline is kept out of that file's one funnel deliberately: `AudioDevices.Open` and `AudioDevices.EngineFormat` run inside a `DeviceOpen` ask that is already counting, and one there would be two deadlines over a single device
- ISC-162 — `DeviceEnquiryTests.A_question_that_has_not_come_back_is_not_put_to_the_machine_again` and `.Two_lookers_that_arrive_together_pay_the_deadline_once_each_and_no_more` (`tests/MeetingTranscriber.Audio.Tests`) green 2026-08-20: with an ask given up on still out there, that question is refused at once, is never put to the machine at all, and the sentence names it. The second is the two callers that really do overlap, on the two questions they really do ask — the screen on its dispatcher and the watcher on its own thread — and it holds what a single-threaded probe cannot: each pays the deadline once because neither is stopped by what the other has not yet learnt, so a deadline paid once is paid once per question. Then that a machine coming back from the later of the two does not free the earlier, which is one thread out of the audio service and one still inside it, while the one that did come back is asked again in the same breath. Red 2026-08-20 with the memory ignored: 4 of the 7 failed and the second look reached the machine and waited five seconds of its own. That the wait resumes once every body is back is asserted by the teardown of every wedging test, which is a look at the question that was wedged and has to succeed — and it has to be that question, since any other one is answered whether or not the wedged body ever came back. What the deadline alone does not buy is why this is a claim and not a detail: a screen asks once a second, so what a person would sit through is the same freeze with pauses in it, one abandoned thread per go. What no probe reaches is two callers of one question at once, because the product has none — the window lists the microphones in its constructor, before any meeting exists, and the watcher lists them only while one is running; that the memory holds more than one entry anyway is a property of a list and is argued where the list is. Narrowed on 2026-08-20 from every question about this machine's devices to the one asked; what it used to say is in `git log -- ISA.md`, and ISC-164 is the half that says why
- ISC-164 — `DeviceEnquiryTests.A_question_still_out_there_leaves_a_different_one_asked` and `.Two_lookers_that_arrive_together_pay_the_deadline_once_each_and_no_more` (`tests/MeetingTranscriber.Audio.Tests`) green 2026-08-20: with the screen's once-a-second question about what this machine plays through given up on and its body still inside the audio stack, the watcher's list of the microphones is put to the machine and comes back at once, and the body it ran is what says the machine answered rather than the clock. Then the same across the seam the watcher actually uses — `AudioDevices.Microphones()` itself, asserted to come back at once and, if it refuses, to refuse in its own right rather than out of this memory, since what a build agent's audio stack answers is nobody's evidence. The wedged question is asserted still refused in the same test, so what is measured is the scope of the memory and not a memory that quietly emptied itself. Written the way round that costs a meeting — the cosmetic question wedged, the recovery the one that has to get through — because reversed it passes over code that couples them. The concurrent form is the second probe, where the two overlap on their own threads and each pays its own deadline. Red 2026-08-20 with the memory unscoped, which is what this branch built first and what the audit held it over: those two failed of the seven, waiting the whole deadline on the microphones because a question about the playback endpoint was out there. Red again, 4 of 7, with the memory ignored altogether. What no probe reaches is the meeting itself, since a microphone unplugged mid-recording needs hardware; what stands in for it is that the watcher's look is this same bounded question, every two seconds, on the thread `CaptureSession` follows a device from
...
- ISC-73.2.1 — four `capture` runs on this machine 2026-08-20, Windows 11 build 26200, against channel 0 obtained from the process loopback rather than from a loopback on the playback endpoint. 20 s with nothing playing at all: 2006 packets 10 ms apart, covering 0:00:20 with nothing lost, loudest silent — where the endpoint it replaces hands over no packets at all while nothing plays into it, so that stretch used to be missing rather than silent. 12 s with a 440 Hz tone: -8.7 dBFS steady every second, 1203 packets. 15 s with another application holding the speakers in `AudioClientShareMode.Exclusive`: the whole recording ran, 1491 packets, 0:00:15 covered, `audio.wav` produced. Channel 0 is silent for all of it, which is what that state is — nothing else on the machine may render while an exclusive client holds the endpoint — so what this run proves is the half the claim turns on and not the whole sentence: the capture opens and goes on delivering where the old design could not have started, since at that same moment the shared playback stream it needed to keep the endpoint awake was refused `AUDCLNT_E_DEVICE_IN_USE` (`0x8889000A`). That the stretch is recorded as the silence it was is the first run's, above. 20 s following a silent `explorer` moved to the whole machine at 12 s: the tone that had been playing throughout appeared on channel 0 the next second, one spool, 0:00:20 covered, the folder's `changes.jsonl` naming the move. `docs/process-capture.md` holds the runs. No test reaches this and none can: what it turns on is what Windows hands a client whose targets are playing nothing, and a build agent has no device to ask. Every one of the four is the `capture` command built and run out of this tree, so what they reach is the mechanism and not the build somebody installs — which is ISC-73.2.2, open, and is why the parent is open with this leaf closed
...
- ISC-121 — `SpoolChangesTests` and `UnfinishedRecordingsTests.A_recording_whose_channel_was_moved_is_found_again_saying_so` (`tests/MeetingTranscriber.Audio.Tests`) green 2026-08-18, and the `capture --process explorer --whole-machine-at 12` run of that day, whose folder came back through `recordings --spool` naming the moment channel 0 moved, what it moved to and what it had been following, beside a card still naming the program it opened on. The `capture --process explorer --whole-machine-at 12` run of 2026-08-20 is the same folder written by a channel 0 that moved onto a process loopback rather than an endpoint. Which of the two channel 0 opened as is ISC-120's and was cited here as well until 2026-08-20, which had one test closing two claims; what this one is about is the folder saying the channel stopped following its program, and from when. This claim used to say a channel 0 that *could not* follow its program says so and says why, closed 2026-08-15 on a card field carrying the reason it fell back; that situation no longer exists, because a program Windows will not follow now stops the recording instead of opening the endpoint, so the field could only ever have been null. What the folder has to keep saying is the same thing one moment later: the channel is no longer the program, and from when
...
- ISC-158.1 — the UI probe on the packaged build 2026-09-01, with the application in English, once for each answer channel 0 can follow. Both walks start on a screen with nothing said on it, where record reads `disabled`, and end with the microphone, what channel 0 follows and what will be spoken each chosen: `see nothing-said choose MicrophonePicker fifine choose SourcePicker <what> choose SpokenPicker English wait RecordButton see answered`, with `<what>` the whole machine's own entry the first time — `Everything this machine plays` — and one program the second. What proves each answer was really there is the `choose` itself: it fails naming every entry of the list when the one asked for is not in it, and it selects nothing else, so a walk that ran is a walk that found that entry. What the pair of trees then proves is that the answer was enough — they differ in the word `disabled` on record and in nothing else, the tree carrying which controls are live rather than what is chosen in them. The whole-machine half had no probe until this pass and the reason was the picker: a ComboBox carousels a list too long to fit and opened this one part way down the alphabet, so the entry above the programs was drawn for neither a reader nor a walk. Reaching an entry a list has not drawn is the probe's own to do and is `Session.Offered` now; opening at the top is the screen's, and `MainWindow.xaml` says why.
- ISC-165.1 — `MeetingRecordingsTests.A_meeting_nobody_named_comes_out_of_recording_with_no_name` (`tests/MeetingTranscriber.Recording.Tests`) green 2026-09-01, and the UI probe: a meeting recorded and never named reads `Unnamed` on the list, which is the catalogue's own two words rather than anything about that meeting. Asserted after stopping and not only after starting, because stopping is where a name would come from if it came from anywhere — the audio has a file, the folder has a path and the run carries two device names. Red 2026-09-01 with a title written at `MeetingRecordings.Open`. The two doors that do carry a title are not counter-examples and are not probed here: bringing audio in and filing a transcribed meeting each take one from whoever typed it.
- ISC-170 — the UI probe on the packaged build 2026-09-01, with the application in English: `see` as the first instruction after it was started and before anything was pressed. One window, and its tree carries both halves — the microphone and source pickers, what will be spoken and record above, and under them the meetings header, the count and a recorded meeting's row with the press its stage offers. Nothing was pressed to reach them, and a list that had arrived in a window of its own would have stopped the walk rather than passed it: the probe refuses to guess which window is the screen once there is more than one.
- ISC-171 — the UI probe on the packaged build 2026-09-01, with the application in English: `see docked press OpennessButton wait OpennessButton see whole press OpennessButton wait OpennessButton see docked-again`. Raised, the recorder half is out of the tree entirely rather than scrolled off it — the pickers, record, pause, carry on and stop are gone, and what is left is the list with the line saying where the meetings are kept, the window's report, its status line and the packaging button around it. Those four stay in both positions on purpose, which is why the claim is the recorder's room and not the whole window. The control that raised it is the same element in the same place, reading `Bring the list back down` where it read `Open the whole list`, and the third tree is the first one line for line apart from the moment it was read.
