...
- ISC-139.1 — `WholeMachineTests` (`tests/MeetingTranscriber.Cli.Tests`) green 2026-08-19, which is the evidence ISC-139 already stood on read as the leaf it always was: the offer is said once however long the program stays silent, a key typed before it appeared is not an answer to it, nor is the second key of two, and answering twice moves once
