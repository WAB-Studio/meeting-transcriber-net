...
- ISC-158.10 — the same walk and ISC-158.7's. A meeting recording and one saving show a size on the list and never a length — `2026-09-02 02:13 · 265.5 MB` — and the saving card carries none; the first tree after reads `2026-09-02 02:13 · 0:06:00`, the figure the report gives. ISC-79's run is sharper: a crashed one read `0:00:50 · 37.0 MB` and became a meeting of `0:00:50`. The halves that walk missed were read rather than probed: `DiagnosticCommands.Status` and `TranscriptHeader` state no length in any state, and `recovery` states the blocks' one before a meeting row has it, which is the preview it exists for. Unreached: a save in flight, which `recovery` cannot tell from a crash — card #86
