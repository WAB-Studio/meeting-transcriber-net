...
- ISC-127 — `ExtractionPositionTests` (`tests/MeetingTranscriber.Infrastructure.Tests`) green 2026-08-16, over `decisions`, `action_items` and `open_questions` alike, and `.A_position_in_an_extraction_belongs_to_one_row_wherever_it_is_stored` reads both halves of the rule off the schema, so a table carrying a position and missing either fails without anybody listing it. Both it and `.One_extraction_cannot_put_two_things_in_the_same_position` red 2026-08-16 against a corpus with `ix_decisions_extraction_run_id_ordinal` dropped, and the sweep red again against the CHECK it looks for. `CorpusRebuildTests.Rebuilding_puts_everything_an_extraction_produced_back_at_its_own_position` holds the other side, with no row id in common across the rebuild
