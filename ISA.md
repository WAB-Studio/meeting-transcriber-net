...
- ISC-152 — `UiTextsTests`, `UiTextTests`, `TextLineTests` and `DeviceLinesTests` (`tests/MeetingTranscriber.Presentation.Tests`), `ScreenTextsTests` and `MeetingCardTextTests` (`tests/MeetingTranscriber.App.Tests`) and `WaitingRecordingsTests` (`tests/MeetingTranscriber.Recording.Tests`) green 2026-09-02, re-run because the claim was false twice: `WaitingRecording.Unrecoverable` handed the meetings list English prose it printed inside a catalogued frame, and the microphone picker read `(default)` in Spanish off `AudioDevice.ToString`. The first closed on `MeetingCardTextTests` reading the reason table, red with one arm dropped; the second on `DeviceLinesTests` for the words and `ScreenTextsTests.No_screen_shows_a_person_what_a_type_says_about_itself` for the screen reaching them, red on the old line restored. Then the UI probe on the packaged build 2026-09-02, the application in Spanish: `choose MicrophonePicker "(default)"` failed listing what is in it — `Micrófono (fifine Microphone) (predeterminado)` and `Micrófono (GENERAL WEBCAM)`. Not reached: a `ToString` behind a method, which is `SourcePicker` today.
