...
- [x] ISC-161: Asking this machine what it can record from comes back at a deadline, even when the audio service never answers.
- [x] ISC-162: Anti: nothing asks this machine another question about its devices while it has not come back from one it was given up on, so a screen that looks every second costs one deadline and not one at every look.
