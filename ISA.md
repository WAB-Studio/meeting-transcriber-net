...
- ISC-77 — three `capture` runs on this machine 2026-08-18, all following `explorer`, which plays nothing. The offer is made at 0:00:10 of every one of them and says what taking it costs. The 24 s run that took it at 12 moved channel 0 from `explorer (pid 24172)` to `Altavoces (High Definition Audio Device)` without the meeting stopping: both spools ran to 0:00:24, `audio.wav` came out one file of 0:00:24 over 387983 frames, and the seam is 27 ms recorded as lost rather than closed up — a real hole of the length the clock says, which is what the whole timeline does with a gap. The 26 s run that took it at 10 had a Windows sound played into the endpoint from second 14: channel 0 reads silent for every second it followed `explorer` and −15.5 dBFS afterwards, so what it carries after the move is what the machine is playing and not merely a stream that opened. `SilentProgramTests` and `SpoolChangesTests` (`tests/MeetingTranscriber.Audio.Tests`) green the same day hold the rule that says a program brought nothing back — the loudest since it opened, ten seconds, and never about a channel already recording the whole machine — and what the folder keeps of the move. What no test reaches is the move itself, which needs two real devices: it is the three runs above
- ISC-139 — the 20 s `capture --process explorer` run on this machine 2026-08-18: the offer was made at 0:00:10 and nobody took it, and the recording ended still on `ProcessLoopback` with its folder holding no changes file at all, so nothing moved it. Before this, `CaptureSession` caught a program Windows would not follow and opened the playback endpoint in its place, noting the reason on the card — a file carrying every notification and every other application on the machine, from a press somebody made asking for one program. That catch is gone: what it is replaced by is the sentence saying what the choice would cost, and the recording does not start. The remaining half is `SilentProgramTests.A_channel_recording_the_whole_machine_is_never_the_wrong_program`, green the same day, which is the offer never being made where taking it would mean nothing. What no probe reaches is a machine that refuses the activation — nothing here can make Windows refuse one — so that the refusal no longer opens a second device is read off `CaptureSession`
