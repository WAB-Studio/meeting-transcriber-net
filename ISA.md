...
- [x] ISC-161: A recording one of whose channels changed device says in its own folder which device fed which stretch of it, and when it changed.
...
- ISC-161 — `SpoolChangesTests` (`tests/MeetingTranscriber.Audio.Tests`) green 2026-08-20 and the `capture --then-microphone` run of that day, whose folder came back through `recordings --spool` naming the moment channel 1 moved, the endpoint it moved to and the one it had been on, beside a card still naming the microphone it opened with. The card is written once and the change is a line beside it, which is ISC-122 read one event wider: a channel that came to name two devices over one meeting says the first on the card and every one after it in the changes. The device is the id and not only the name, because two identical headsets share a name and nothing reopens one by it. `.A_channel_zero_is_never_said_to_have_moved_onto_a_device` is the anti, refused where it is written and where it is read, since a folder is five keys of JSON a person is invited to look at. What no probe reaches is the moment itself for a device Windows took away rather than one somebody chose — the line the folder gets is the same line either way, and which of the two made it is not something the folder records or needs to
