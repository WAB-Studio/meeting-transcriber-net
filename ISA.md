...
- ISC-158.1 — `RecorderScreenTests` (`tests/MeetingTranscriber.Recording.Tests`) green 2026-09-01 over the rule, and the UI probe over the screen: on the packaged build the front door carries both pickers, record reads `disabled`, and it stops reading `disabled` only once the microphone, what channel 0 follows and what will be spoken have each been answered — three `choose` steps and two trees whose only difference is that one word. The rule half is what says a choice survives the machine being asked again: a microphone still there stays chosen as the machine now describes it, one that has gone stops being an answer, and record goes dead with it. What the probe cannot reach is the whole machine as the thing channel 0 follows: the source picker holds every process playing on this machine and virtualises them, so the entry sitting above them is not realised for the probe to name, and the program that was chosen instead is the same answer through the same picker.
