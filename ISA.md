...
- ISC-139.1 — the 16 s `capture --process explorer` run on this machine 2026-08-18: the offer was made at 0:00:10, nobody took it, and the recording ended still on `ProcessLoopback` its folder holding no changes file at all. Its microphone heard the room throughout, which says a silent channel 0 was the program and not a dead recording. `SilentProgramTests.A_channel_recording_the_whole_machine_is_never_the_wrong_program` (`tests/MeetingTranscriber.Audio.Tests`) green the same day is the offer never made where it would mean nothing, and `WholeMachineTests` (`tests/MeetingTranscriber.Cli.Tests`) green 2026-08-18 holds the gate with no device in it, under ten mutations one at a time, each red at the test named for it. That a refusal opens no second device is read off `CaptureSession`
...
- ISC-152 — `UiTextsTests`, `.The_catalogue_holds_what_the_application_says`, `.Reading_in_one_language_leaves_nothing_in_the_other`, `UiTextTests` and `TextLineTests` (`tests/MeetingTranscriber.Presentation.Tests`), and `ScreenTextsTests` (`tests/MeetingTranscriber.App.Tests`) green 2026-08-18, with `.No_screen_prints_what_a_machine_said_on_a_line_of_its_own` added 2026-08-20 for the half no walk of a catalogue can see. Red 2026-08-18 with one entry's English version shortened from `Package: {0}` to `Package`, four failing and naming what the two versions disagree about; and red three times over on the app's markup — `Text="Listo"` planted as an attribute, `<TextBlock>Listo</TextBlock>` between tags, and the heading's binding swapped for `{StaticResource Greeting}` — each naming the file and what it found
