...
- [x] ISC-160: A meeting whose audio had its channels averaged into one on the way in still says so once the command that brought it in is gone.
...
- ISC-160 — `AudioIntakeTests.What_became_of_the_channels_is_still_in_the_corpus_afterwards` (`tests/MeetingTranscriber.Recording.Tests`) green 2026-08-20. Two meetings brought into one corpus — one averaged from a pair, one that only ever had a single track — read back through a second connection, and the probe asserts first that they agree everywhere else: same profile, same length, cards identical field for field. That is what makes the audit line load-bearing rather than decorative, and it is why the probe reads rows instead of reading the report. Without it the only account of a meeting having lost the split between what the machine played and what the microphone heard is a line of console output from the afternoon somebody ran the command, and ‘found out about later’ is exactly what would happen. Red 2026-08-20 with the audit line saying only where the file was: 1 of 90 recording tests failed and the rest stayed green. What is argued rather than probed is that free text in the audit trail is enough — a column would be stronger, and it is what a second reader of this fact would want
