...
There is no installed build and no corpus anybody keeps. So no change has to carry old data
forward: a migration is free to drop what it replaces and say so, a column can be renamed for the
name it should have had, and none of it needs a compatibility path, a fallback or a version check.
Do the thing that is right now, not the thing that keeps a hypothetical old corpus readable.

The Python system is the one exception, and it is not one really: it is read by a tool in `tools/`
that never migrates anything and gets deleted whole. Nothing under `src/` is allowed to know it
existed.

When that stops being true — the first install somebody records into — this section is what gets
deleted, and only migrations written after that point owe anybody anything.
...
- Migration ids here are hand-picked and ahead of the clock, so a freshly generated one sorts
  *before* the ones already committed and would run in the wrong order. Rename the file, its
  designer and the `[Migration]` attribute inside it to the next free slot before anything else.
- SQLite rebuilds the whole table for almost any schema change, and a composite foreign key onto a
  UNIQUE constraint that the same rebuild is adding cannot resolve: the new table references the
  old one by name, and the old one has no such key yet. `nodes` pointing at its own parent is
  exactly that, and it fails with `foreign key mismatch`. It takes two migrations — the columns and
  the unique constraints first, the foreign key second.
- `dotnet-ef` loads the freshly built assembly into its own process, so Smart App Control blocks it
  outright: `Could not load assembly`, wrapping `An Application Control policy has blocked this
  file` (0x800711C7). It reads like an EF bug and is not one, and `dotnet build` and `dotnet test`
  are unaffected, which is what makes it confusing. `Get-WinEvent -LogName
  Microsoft-Windows-CodeIntegrity/Operational` names the blocked file, and
  `(Get-ItemProperty HKLM:\SYSTEM\CurrentControlSet\Control\CI\Policy).VerifiedAndReputablePolicyState`
  says whether it is on. The fix is to turn it off; failing that, a migration, its designer and the
  model snapshot can be written by hand, and `HasPendingModelChanges` in `CorpusMigrationTests` is
