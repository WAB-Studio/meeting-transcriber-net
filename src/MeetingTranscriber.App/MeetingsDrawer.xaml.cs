...
/// state for this one. A process that goes during a keep leaves nothing of the finish or the whole
/// of it — its audio row, its length and its run's end are one commit — and possibly a capture run
/// never marked as recovered, which is a save of its own after the finish. The one exception is a
/// rename that outran that commit, which leaves the audio file in the folder with no row naming it
/// and a meeting the application then refuses to finish.
/// </para>
/// <para>
