...
if (Test-Path $analyzerDll) {
    ...
    #
    # The Test-Path below only looks in the project folder, but MSBuild stops at the FIRST
    # Directory.Build.props it finds walking up from the project. In a multi-project repo the
    # shared one lives at the root, so this file lands *below* it and shadows it: the project
    # silently loses Nullable / LangVersion / ImplicitUsings for the duration of the build, and
    # ImplicitUsings dropping out shows up as CS0246 on usings that compile fine otherwise.
    # Hence the explicit import of the props above — it restores the inheritance MSBuild cut.
    ...
    # Only create if one doesn't already exist in the project folder (don't overwrite user's file)
...
"@ | Set-Content $tempBuildProps
        ...
        Write-Host "--> Microsoft.WindowsAppSDK.Analyzers: skipped (Directory.Build.props already in the project folder)" -ForegroundColor DarkGray
    }
}
