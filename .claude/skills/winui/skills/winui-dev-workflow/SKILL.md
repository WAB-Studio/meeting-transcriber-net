...
```
```xml
<!-- src/MeetingTranscriber.App/MeetingTranscriber.App.csproj -->
<PackageReference Include="CommunityToolkit.Mvvm" />
```

`dotnet add package <Name>` produces exactly that — it is CPM-aware on the SDK this repo targets
(verified on 10.0.302): it writes the versionless `<PackageReference>` into the `.csproj` and a
`<PackageVersion>` into `Directory.Packages.props`, inserted into the existing `<ItemGroup>` in
alphabetical order, which is the order that file is already kept in. Upstream's "never pass
`--version`" advice holds.

Two caveats that are specific to this repo:

- **Never pass `--no-restore`.** Without a restore the CLI cannot resolve a version and writes
  `Version="*"`, a floating wildcard. Every version in `Directory.Packages.props` here is exact
  and several are deliberately held back.
- **The CLI does not write the comment.** Several pins carry a comment explaining why that exact
  version — `Microsoft.Testing.Extensions.CodeCoverage` held at 18.0.x because xunit.v3 3.2.2
  needs Microsoft.Testing.Platform 1.x, `SQLitePCLRaw.bundle_e_sqlite3` at 3.x against
  GHSA-2m69-gcr7-jv3q. If a package needs a non-latest version, add the reason by hand.

`CentralPackageTransitivePinningEnabled` is on, so a `<PackageVersion>` also pins transitives.
That is what holds the `SQLitePCLRaw` pin above in place — do not remove a pin to make a restore
conflict go away.
